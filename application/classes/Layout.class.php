...
class Layout
{
     ...
     * @var array
     */
    protected $_meta = array('<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">');
     ...
     * @var string
     */
    public $title = 'École Polytechnique - Logement des admissibles';
     ...
     * @access protected
     */
    protected $_libraries = array('jquery/jquery-1.9.1.js', 'jquery/jquery-ui-1.10.1.custom.min.js', 'jquery/jquery.visited.js', 'jquery/jquery.ui.datepicker-fr.js');
     ...
     * @param integer $type le type d'élément à ajouter (css ou js)
     * @param integer $placement le placement (prepend ou append)
     * @return void
     */
    protected function _add($sElement, $type, $placement = self::Append)
    {
        $element = $this->_generateUrl($sElement, $type);
        switch ($type) {
            //si c'est un css
            case self::Css:
     ...
     * @param integer $nType type de l'élément à générer (css ou js)
     * @return string
     */
    protected function _generateUrl($sUrl, $nType)
    {
        if ($nType === self::Js) {
    ...
    public function renderHead()
    {
        ...
        return $sHead . '<title>' . $this->title . '</title>' . "\n" . '</head>';
    }
    ...
    public function renderContent()
    {
        ...
        if ($this->_content !== array() && count($this->_content) > 0) {
